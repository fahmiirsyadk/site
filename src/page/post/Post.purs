...
homeArrowIcon :: forall message. FK.Child message
...
chainLinkIcon :: forall message. FK.Child message
...
checkIcon :: forall message. FK.Child message
...
view input =
  HH.article [ HP.class_ "min-w-0 space-y-6 overflow-x-auto" ]
        [ if input.post.banner == "" then FK.empty else cover input
        , HH.div [ HP.class_ "flex w-full items-center gap-3 text-[12px] leading-[1.7]" ]
            ...
            , HH.span [ HP.attr "data-relative-date" input.post.date, HP.class_ "shrink-0 text-right text-neutral-500" ] [ HH.text input.post.dateLabel ]
            ]
        , HH.div
...
cover :: forall message. Input message -> FK.Child message
cover input =
  if isNativeMedia input.post.banner then
    HH.div [ HP.class_ "post-cover overflow-hidden rounded-lg" ]
      [ if isMp4 input.post.banner then
          HH.video
            [ HP.attr "src" input.post.banner
            , HP.attr "autoplay" ""
            , HP.attr "muted" ""
            , HP.attr "loop" ""
            , HP.attr "playsinline" ""
            , HP.attr "preload" "metadata"
            , HP.attr "aria-label" input.post.title
            , HP.class_ "post-cover-media"
            ] []
        else
          HH.img
            [ HP.attr "src" input.post.banner
            , HP.attr "alt" input.post.title
...
copyIcon :: forall message. PostModel.CopyStatus -> FK.Child message
