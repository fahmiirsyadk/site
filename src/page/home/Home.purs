...
thinkingAbout mount =
  HH.span
    [ HP.dataAttribute "random-scribble" "true"
    ...
    , HH.svg
        [ HP.ariaHidden true
        , HP.viewBox "0 0 132 72"
        , HP.class_ "thinking-scribble-svg"
        ]
        [ HH.path
            [ HP.dataAttribute "random-scribble-path" "true"
...
introduction :: forall message. Input message -> HH.Child message
introduction input =
  HH.section [ HP.class_ "w-full" ]
    [ HH.div [ HP.class_ "max-w-[32rem] space-y-3 text-[13px] leading-[1.7] text-[#171717] dark:text-neutral-200" ]
        ...
        , HH.p []
            ...
            , HH.text " or email "
            , HH.a [ HP.href ("mailto:" <> Site.emailAddress), HP.class_ "inline-flex items-center gap-1 underline decoration-dotted decoration-neutral-400 underline-offset-4 hover:text-[#FF4B26]" ] [ Icon.mail, HH.text Site.emailAddress ]
            , HH.text "."
            ]
        ]
    ]

latestPosts :: forall message. Array Post -> HH.Child message
latestPosts posts =
  PostList.view
    { posts
