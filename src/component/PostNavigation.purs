...
view :: forall message. { previous :: Array Link, next :: Array Link } -> FK.Child message
view input =
  if Array.null input.previous && Array.null input.next then
    FK.empty
  else
    HH.nav
      [ HP.attr "aria-label" "Post navigation"
...
slot :: forall message. Array Link -> FK.Child message
slot links =
  case Array.head links of
    Nothing -> HH.span [] []
    Just link ->
      HH.a
        [ HP.attr "href" link.path
